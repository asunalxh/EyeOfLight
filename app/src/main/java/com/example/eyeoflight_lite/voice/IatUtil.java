...
public class IatUtil {
    ...
    public interface IatUtilListener {
        void onResult(String resultString);
        void onVolumeChanged(int volume, byte[] data);
    }

    private static IatUtilListener iatUtilListener = null;


    private static Context context;

    private final static String TAG = "IatUtil_Log";
    // 语音听写对象
    ...
    // 用HashMap存储听写结果
    private static StringBuffer mIatResults = new StringBuffer();
    ...
    public static void startListen(boolean isShowDialog, IatUtilListener mIatUtilListener) {
        // 开始听写
        // 如何判断一次听写结束：OnResult isLast=true 或者 onError

        iatUtilListener = mIatUtilListener;
    ...
    private static RecognizerListener mRecognizerListener = new RecognizerListener() {
        ...
        public void onResult(RecognizerResult results, boolean isLast) {
            ...
            if (ls && iatUtilListener != null)
                iatUtilListener.onResult(mIatResults.toString());
        }
...
//            Log.d(TAG, "返回音频数据：" + data.length);
            if(iatUtilListener !=null)
                iatUtilListener.onVolumeChanged(volume,data);
        }
