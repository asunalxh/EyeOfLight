...
package com.example.eyeoflight_lite.tracking;
...
//    logger.i("Processing %d results from %d", results.size(), timestamp);
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        for (final TrackedRecognition recognition : trackedObjects) {
            ...
            final String depthString = recognition.depth == 0.f ? "" : (String.valueOf(recognition.depth) + "m");
            borderedText.drawText(
                    canvas, trackedPos.left + cornerSize, trackedPos.top, labelString + "%" + depthString, boxPaint);
        }
    }
...
//      logger.v("Nothing to track, aborting.");
        ...
        for (final Pair<Float, Classifier.Recognition> potential : rectsToTrack) {
            ...
            trackedRecognition.depth = potential.second.getDepth();
